...
import {IndexerStargateClient} from "./indexer_startgateclient";
...
export const createIndexer = async () => {
    ...
    const handleTx = async (indexed: IndexedTx) => {
        try {
            ...
            // Skipping if the handling failed.
            console.log(`Failed to handle tx ${indexed.hash} with logs: ${indexed.rawLog}`)
        }
    }
